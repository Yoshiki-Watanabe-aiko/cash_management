...
$docsPattern = '^(docs/requirements\.md$|docs/詳細設計書/|CONTEXT\.md$|docs/adr/|docs/課題管理表\.md$)'
...
if ($sourceChanged -and -not $docsChanged) {
    ...
    $message = "[doc-check] backend/app・backend/alembic・frontend/src に変更がありますが、docs/requirements.md / docs/詳細設計書 / CONTEXT.md / docs/adr / docs/課題管理表.md はいずれも変更されていません。該当する場合はドキュメントも更新してください(仕様に影響しない変更であれば無視して構いません)。変更ファイル: $fileList"
